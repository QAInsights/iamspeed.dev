...
import { sambanovaAdapter } from "../../src/lib/providers/sambanova";
...
describe("sambanova-adapter", () => {
  ...
  it("calls the SambaNova endpoint with correct headers", async () => {
  ...
  it("strips sambanova/ prefix from model id", async () => {
    const fetchSpy = vi.fn().mockResolvedValue({
      ok: true,
      body: createSSEStream([
        JSON.stringify({ choices: [{ delta: { content: "Hi" } }] }),
