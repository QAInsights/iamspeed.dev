...
/**
 * Reusable social share button row. Takes pre-built share text so any caller
 * (single-model ShareBar, race podium, future features) can share arbitrary
 * results without reimplementing the icon buttons, popups, or clipboard logic.
 */
...
export function ShareButtons({ shareText, ariaLabel = "Share results", tps, ttft, provider, model }: ShareButtonsProps) {
  ...
  const shareLinks = [
    ...
    { label: "Share on Reddit", Icon: RedditIcon, url: `https://www.reddit.com/submit?title=${encodedText}&url=${encodedUrl}` },
    { label: "Share on Facebook", Icon: FacebookIcon, url: `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}&quote=${encodedText}` },
  ];
